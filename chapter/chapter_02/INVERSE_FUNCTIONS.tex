\clearpage
\section[\textsc{inverse functions}]{INVERSE FUNCTIONS}\index{Function!inverse|(}\index{Inverse function|(} 
...
\begin{problems}
    \problem[*]{Let $A \subset \F{R}^n$ be an open set and $f\colon{}A\to \F{R}^n$
        a continuously differentiable 1-1 function such that  $\det f'(x)\neq 0$
        for all $x$. Show that $f(A)$ is an open set and $f^{-1}\colon{}f(A)\to A$ is 
        differentiable. Show also that $f(B)$ is open for any open set $B\subset A$.
        }
    \problem{
        \begin{enumerate}[label={\upshape(\alph*)}]
            \item Let $f\colon{}\F{R}^2\to \F{R}$ be a continuously differentiable function.
                Show that f is not 1-1. \textit{Hint:} If, for example, $\R{D}_1f(x, y)\neq 0$
                for all $(x, y)$ in open set $A$, consider $g\colon{}A\to \F{R}^2$ defined by $g(x, y) = (f(x, y), y)$
            \item Generalize this result to the case of a continuously differentiable 
                function $f\colon{}\F{R}^n\to \F{R}^m$ with $m< n$.
        \end{enumerate}
        }
    \problem{
        \begin{enumerate}[label={\upshape(\alph*)}]
            \item If $f\colon{}\F{R}\to \F{R}$ satisfies $f'(a)\neq 0$ for all $a\in \F{R}$, show that 
                $f$ is 1-1 (on all of $\F{R}$)
            \item Defined $f\colon{}\F{R}^2 \to \F{R}^2$ by $f(x, y) = \left(\mathrm{e}^x \cos y, \mathrm{e}^x\sin y\right)$.
                Show that $\det f'(x, y)\neq 0$ for all $(x, y)$ but $f$ is not 1-1. 
        \end{enumerate}
        }
    \problem{Use the function $f\colon{}\F{R}\to \F{R}$ defined by 
        \begin{align*}
