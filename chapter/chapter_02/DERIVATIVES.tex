...
\begin{theorem}
    Let $g_1, \cdots, g_m:\F{R}^n\to \F{R}$ be continuously differentiable\index{Differentiable function!continuously}\index{Function!continuously differentiable} at $a$, and 
...
Theorem 2-9 is often called the \textbf{chain rule}\index{Chain rule}, but is weaker
...
and we can write 
\begin{align*}
    F(x, y) = f\left(g(x, y), \bar{h}(x, y), \bar{k}(x, y)\right).
\end{align*}
...
down the function $\bar{h}$ and $\bar{k}$
...
\begin{problems}
    ...
    \problem{Let $f\colon{}\F{R}^n\to \F{R}$. For $x\in \F{R}^n$, the limit 
            \begin{align*}
                \lim_{t\to 0}{\frac{f(a+tx) - f(a)}{t}},
            \end{align*}
            if exists, is denoted $\R{D}_xf(a)$, and called the \textbf{directional derivative}\index{Derivative!directional}\index{Directional derivative} of $f$ at $a$,
            in the direction $x$.
            \begin{enumerate}[label={\upshape(\alph*)}]
    ...
    \problem{Let $f\colon{}\F{R}^2\to \F{R}$ be defined as in Problem 2-26. Show that $\R{D}_xf(0, 0)$ exists for all 
        $x$ , although $f$ is not even continuous at $(0, 0)$.}
    \problem{
        \begin{enumerate}[label={\upshape(\alph*)}]
            \item Let $f\colon{}\F{R}\to \F{R}$ be defined by 
                \begin{align*}
            ...
            \item Let $f\colon{}\F{R}^2\to \F{R}$ be defined by 
                \begin{align*}
    ...
    \problem{A function $f\colon{}\F{R}^n\to \F{R}$ is \textbf{homogeneous}\index{Homogeneous function}\index{Function!homogeneous} of degree $m$ if $f(tx) = t^mf(x)$ 
    ...
    \problem{If $f\colon{}\F{R}^n\to \F{R}$ is differentiable and $f(0) = 0$, prove that there exists $g_i:\F{R}^n\to \F{R}$
        such that 
        \begin{align*}
