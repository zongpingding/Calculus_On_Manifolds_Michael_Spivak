...
\begin{theorem}
    \begin{enumerate}[label={\upshape(\arabic*)}]
        \item If $f:\F{R}^n\to \F{R}^m$ is a \Index{Constant function}\index{Function!constant} (that is, if for some
...
\begin{problems}
    ...
    \problem{Find $f'$ for the following (where $g:\F{R} \to \F{R}$ is continuous):\vspace*{-1em}
        ...
        \end{enumerate}\end{multicols}}
    \problem{A function $f:\F{R}^n\times \F{R}^m \to \F{R}^p$ is \textbf{bilinear}\index{Bilinear function} for $x, x_1, x_2 \in \F{R}^n$
