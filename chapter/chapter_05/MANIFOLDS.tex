\section[\textsc{manifolds}]{MANIFOLDS}
...
a \textbf{diffeomorphism}\index{Diffeomorphism}. (``differentiable'' henceforth means ``$C^\infty$''.)

A subset $M$ of $\F{R}^n$ is called a $k$-\textbf{dimensional manifold}\index{Manifold}\index{Dimension!of a manifold} (in $\F{R}^n$)
...
One common example of an $n$-dimensional manifold is the $n$-\textbf{sphere}\index{Sphere}
...
\begin{theorem}
    ...
    each point $x\in M$ the following ``\textbf{coordinate condition}''\index{Coordinate condition} is satisfied:
...
[Such a function $f$ is called a \textbf{coordinate system}\index{Coordinate system} around $x$ (see Figure \ref{Fig 5-2}).]
...
The \textbf{half-space}\index{Half-space} $\F{H}^k\in\F{R}^k$ is defined as $\{x\in\F{R}^k:x^k\ge 0\}$. A subset 
$M$ of $\F{R}^n$ is a $k$-\textbf{dimensional manifold-with-boundary}\index{Manifold-with-boundary}\index{Dimension!of a manifold-with-boundary} (Figure \ref{Fig 5-3}) if for evry point 
...
\begin{problems}
    ...
    \problem{
        ...
        function $g\colon{}A\to\F{R}^{n-k}$ such that $A\cap M=g^{-1}(0)$ and $g'(y)$ has $\R{rank}\; n-k$ when 
        $g(y)=0$.
    }
    \problem{
    ...
    \problem{
        If $f\colon{}\F{R}^n\to\F{R}^m$, the \textbf{graph}\index{Graph} of $f$ is $\{(x,y):y=f(x)\}$. Show that 
    ...
    \problem{
        ...
        show that $N$ is a $(k+1)$-dimensional manifold. Example: the \textbf{torus}\index{Torus} (Figure \ref{Fig 5-4}).
        \begin{figure}[!htb]
            \centering
