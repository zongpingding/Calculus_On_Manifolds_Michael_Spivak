...
orientation, also known as the \textbf{counterclockwise orientation}\index{Orientation!counterclockwise}\index{Counterclockwise orientation}.)
...
\begin{theorem}[Stoke's Theorem]\index{Stokes' Theorem}
...
the fact that the fluid is incompressible\index{Incompressible fluid}. If M is a disc, then $\int_{\partial M} \langle F,T\rangle\;\dd S$
measures the amount that the fluid curls around the center of the disc. If this is zero for all discs, 
then $\nabla \times F=0$, and the fluid is called \textit{irrotational}\index{Irrotational fluid}.
...
the terminology rotation of $F$\index{Rotation of $F$}; this unfortunate term suggested the abbreviation
...
is true for a square, and the Divergence Theorem\index{Divergence of a field} is true for a cube. 
...
\begin{problems}
    ...
    \problem{
        ...
        all directions, we define the \textbf{buoyant force}\index{Buoyant force} on $M$, due to the fluid, 
        as $-\int_{\partial M}\langle F,n\rangle\;\dd A$. Prove the following theorem.
        \textit{Theorem (Archimedes)}\index{Archimedes}. The buoyant force on $M$ is equal to the
