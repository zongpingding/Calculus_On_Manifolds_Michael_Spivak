...
\indexsee{Field}{Vector field}
\indexsee{Form}{Differential form}
\indexsee{Normal}{Outward unit normal}
...
is called the \textbf{tangent space}\index{Tangent space} of $M$ at $x$ (see Figure \ref{Fig 5-5}).
...
a vector $F(x)\in M_x$ for each $x\in M$; such a function is called a \textbf{vector field on}\index{Vector field!on a manifold} $M$. 
There is still a unique vector field $G$ on $W$ such that $f_*(G(a)) = F(f(a))$ for $a\in W$; we 
define $F$ to be \textbf{differentiable}\index{Differentiable vector form!on a manifold}\index{Vector field!on a manifold!differentiable} if $G$ is differentiable. Note that our definition does not depend 
...
tangent space $M_x$ of a manifold $M$\index{Differential form, on a manifold!differentiable}\index{Differential form, on a manifold}\index{Differentiable differential form!on a manifold}. Such choices are called
\textbf{consistent}\index{Consistent choices of orientation}\index{Orientation!consistent choices of} (Figure \ref{Fig 5-6}) provided 
...
for one, and hence for every $a\in W$, then $f$ is called \textbf{orientation-preserving}\index{Orientation-preserving}.
...
A manifold for which orientations $\mu_x$ can be chosen consistently is called \textbf{orientable}\index{Orientable manifold}, 
and a particular choice of the $\mu_x$ is called an \textbf{orientation}\index{Orientation} $\mu$ of $M$.
A manifold together with an orientation $\mu$ is called an \textbf{oriented} manifold\index{Oriented manifold}.
The classical example of a non-orientable manifold is the \textbf{M\"obius strip}\index{M\"obius strip}.
...
This unit vector is called the \textbf{outward unit normal}\index{Unit outward normal}\index{Outward unit normal} $n(x)$; 
...
\textbf{induced orientation}\index{Orientation!induced}\index{Induced orientation}. If we apply these definitions 
...
the \textbf{outward unit normal}\index{Outward unit normal}\index{Unit outward normal} to $M$ (denoted by $\mu$). The vectors $n(x)$ vary continuously on $M$, in an 
...
impossible pn the M\"obius strip\index{M\"obius strip}. In the paper model of the M\"obius strip the two side of the paper (which 
...
famous property of the paper model. The paper model is \textbf{one-sided}\index{One-sided surface} (if you start to paint it on one 
...
\begin{problems}
    ...
    \problem{
        ...
        defined is the \textbf{usual orientation}\index{Orientation!usual} of $M$). If $x\in\partial M$, show that
    ...
    \problem{
        Let $g\colon{}A\to\F{R}^p$ be as in Theorem 5-1.
        \begin{enumerate}[label=(\alph*)]
            \item If $x\in M=g^{-1}(0)$, let $h\colon{}U\to\F{R}^n$ be the essentially unique diffeomorphism
    ...
    \problem{
        ...
        there is an open set $A\subset \F{R}^n$ and a differentiable $g\colon{}A\to\F{R}^1$
    ...
    \problem{
        ...
        $M$ is not). What is $M(\varepsilon)$ if $M$ is the M\"obius strip\index{M\"obius strip}?
    }
    \problem{
        Let $g\colon{}A\to\F{R}^p$ be as in Theorem 5-1. If $f\colon{}\F{R}^n\to\F{R}$ is differentiable and the 
        ...
        \textbf{constraints}\index{Constraints} $g^i=0$. One can attempt to find $a$ by solving the system of equations \eqref{eq:5-2-p16}.
        ...
        leave the equation $g(a)=0$ for last. This is \textbf{Lagrange's method}\index{Lagrange's method}, and 
        the useful but irrelevant $\lambda$ is called a \textbf{Lagrangian multiplier}\index{Lagrangian multiplier}.
    ...
    \problem{
        \begin{enumerate}[label=(\alph*)]
            \item Let $T:\F{R}^n\to\F{R}^n$ be self-adjoint with matrix $A=(a_{ij})$, so 
                that $a_{ij}=a_{ji}$. If $f(x) = \langle Tx, x\rangle = \Sigma_{}^{}{a_{ij}x^ix^j}$, 
                show that $\R{D}_kf(x) = 2 \Sigma_{j=1}^{n }{a_{kj}x^j}$. By considering the maximum of 
