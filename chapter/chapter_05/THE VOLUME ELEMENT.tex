...
\begin{problems}
    ...
    \problem{
        \begin{enumerate}[label=(\alph*)]
            ...
            \item If $c:[0, 2\pi]\times (-1, 1)\to \F{R}^3$ is defined by $c(u,v) = (2\cos u + v\sin(u/2)\cos u, 2\sin u+ v\sin(v/2)\sin u, v\cos u/2)$,
                show that $c([0, 2\pi]\times(-1,1))$ is a M\"obius strip and find its erea.
        \end{enumerate}
    }
    \problem{
    ...
    \problem{
        ...
        \begin{enumerate}[label=(\alph*)]
            ...
            \item Let $M\subset\F{R}^3-\{0\}$ be a compact two-dimensional manifold-with-boundary 
                ...
                The \textbf{solid angle}\index{Angle!solid} subtended by $M$ is defined as the area of $C(M)\cap S^2$, or 
    ...
    \problem{
        Let $f,g:[0,1]\to\F{R}^3$ be the nonintersecting closed curves. Define the linking
        number $l(f,g)$ of $f$ and $g$ be (cf Problem 4-34)
        \begin{align*}
