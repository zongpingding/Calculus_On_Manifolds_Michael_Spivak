\section[\textsc{algebraic preliminaries}]{ALGEBRAIC PRELIMINARIES}
...
A multilinear function\index{Multilinear function} $T:V^k\to\F{R}$ is called a $k$-\textbf{tensor}\index{Tensor} on $V$
...
If $S\in \K{J}^k(V)$ and $T\in \K{J}^l(V)$, we define the \textbf{tensor product}\index{Tensor product}
...
we define an \textbf{inner product}\index{Inner product} on $V$ to be a 2-tensor $T$ such that $T$
is \textbf{symmetric}\index{Symmetric}, that is $T(v, w) = T(w, v)$ for $v, w\in V$ and such that 
$T$ is \textbf{positive definite}\index{Positive definiteness}, that is, $T(v, v)>0$ if $v\neq 0$. We distinguish
$\langle \cdot \rangle$ as the \textbf{usual inner product}\index{Inner product!usual} on $\F{R}^n$. The following 
...
A $k$-tensor $\omega\in \K{J}^k(V)$ is called \textbf{alternating}\index{Tensor!alternating}\index{Alternating tensor} if
\begin{align*}
...
that the sign of a permutation $\sigma$, denoted $\sgn\sigma$\index{Sign of a permutation}, 
...
Either of these two groups is called an \textbf{orientation}\index{Orientation} for $V$.
...
In $\F{R}^n$ we define the \textbf{usual oritation}\index{Orientation!usual} as $[e_1, \cdots, e_n]$.
...
\end{align*}
\vskip-.7em
In other words,if $A^T$ denoted the transpose\index{Matrix!transpose of} of the matricx of the matrix $A$, then 
...
such that $\mu=[v_1,\cdots,v_n]$. This unique $\omega$ is called the \textbf{volume element}\index{Volume element}
...
This $z$ is denoted $v_1\times\cdots\times v_n$ and called the \textbf{cross product}\index{Cross product} of 
...
\begin{problems}
    ...
    \problem{
        If $f\colon{}V\to V$ is a linear transformation and $\dim V=n$, then $f^*:\Lambda^n(V)\to \Lambda^n(V)$
    ...
    \problem{
        ...
        $f\colon{}\F{R}^n\to V$ is an isomorphism such that $f^*T = \langle \cdot \rangle$ and 
    ...
    \problem{
        If $T$ is an inner product on $V$, a linear transfromation $f\colon{}V\to V$ is called \textbf{self-adjoint}\index{Self-adjoint}
