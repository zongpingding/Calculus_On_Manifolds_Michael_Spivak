...
we define a $(k+1)$-form $\dd\omega$, the \textbf{differentiable} of $\omega$, by 
\begin{align*}
    \dd\omega 
...
A form $\omega$ is \textbf{closed} if $\dd\omega = 0$ and \textbf{exact} if $\omega=\dd\eta$,
...
\begin{problems}
    ...
    \problem{
        Let $c$ be a differentiable curve in $\F{R}^n$, that is, a differentiable function 
        $c:[0,1]\to \F{R}^n$. Define the tangent vector $v$ of $c$ at $t$ 
