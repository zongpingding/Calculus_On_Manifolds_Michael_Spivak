\section[\textsc{basic definitions}]{BASIC DEFINITIONS}
...
Recall that a partition\index{Partition!of a closed interval} $P$ of a closed interval $[a,b]$ is a
...
A \textbf{partition}\index{Partition!of a closed rectangle} of a rectangle $[a_1, b_1]\times \cdots \times [a_n, b_n]$
...
and let $v(S)$ be the volume of $S$ (the \textbf{volume}\index{Volume} of a rectangle 
$[a_1, b_1]\times \cdots\times [a_n, b_n]$, and also of $(a_1, b_1)\times \cdots\times (a_n, b_n)$,
is defined as $(b_1-a_1)\cdots (b_n-a_n)$.) The \textbf{lower}\index{Lower sum} and \textbf{upper sums}\index{Upper sum} of $f$ 
...
\begin{lemma}
    Suppose the partition $P'$ refines\index{Refine a partition} $P$ (that is, each subrectangle of $P'$
...
bound of all upper sums for $f$. A function $f\colon{}A \to \B{R}$ is called \textbf{integrable}\index{Integrable function}\index{Function!integrable} 
on the rectangle $A$ if $f$ is bounded and $\sup\{L(f,P)\} = \inf \{U(f, P)\}$. 
This common number is then denoted $\int_{A }^{}{f}$, and called the \textbf{integral}\index{Integrable} of 
...
\begin{problems}
    \problem{
        Let $f\colon{}[0,1]\times [0,1]\to \B{R}$ be defined by
        \begin{align*}
            f(x, y) = 
            \left\{\begin{aligned}
    ...
    \problem{Let $f\colon{}A\to \B{R}$ be integrable and let $g=f$ except at finitely many points.
    Show that $g$ is integrable and $\int_{A }^{}{f} = \int_A g$.}

    \problem{Let $f,g\colon{}A\to \B{R}$ be integrable. 
    ...
    \problem{
        Let $f\colon{} A \to \B{R}$ and let $P$ be a partition of $A$. Show that $f$ is 
    ...
    \problem{
        Let $f,g\colon{}A\to \B{R}$ be integrable and suppose $f\le g$. Show that 
        $\int_A f\le \int_A g$. 
    }
    \problem{
        If $f\colon{}A\to \B{R}$ is integrable, show that $|f|$ is integrable and 
        $|\int_A f|\le \int_A |f|$.
    }
    \problem{
        Let $f\colon{}[0,1]\times [0,1]\to \B{R}$ be defined by 
