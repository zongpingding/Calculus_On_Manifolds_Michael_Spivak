...
If $C\subset A$ for some closed rectangle $A$ and $f\colon{}A\to \F{R}$ is bounded, then 
...
\begin{problems}
    \problem{
        Show that if $f\cdot g\colon{}A\to \F{R}$ are integrable, so is $f\cdot g$.
    }
    \problem{
    ...
    \problem{
        If $f\colon{}A\to \F{R}$ is non-integrable and $\int_A \chi_C = 0$, show that $\{x:f(x)\neq 0\}$
    ...
    \problem{
        Show that an increasing function $f\colon{} [a,b] \to \F{R}$ is integrable on $[a,b]$.
    }
    \problem{
