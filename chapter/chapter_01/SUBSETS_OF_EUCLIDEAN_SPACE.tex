...
This is the \textbf{closed rectangle}\index{Rectangle (closed or open)} $[a,b]\times [c,d]$, 
...
is called a \textbf{closed rectangle}\index{Closed rectangle} in $\F{R}^n$, while the set 
$(a_1, b_1)\times \cdots \times (a_n,b_n)\subset \F{R}^n$
is called \textbf{open rectangle}. More generally a set $U\in\mathbb{R}^n$ is called open (\figref{Fig 1-1})\index{Open set} 
if for each $x\in U$ there is an open rectangle $A$ such that $x\in A\subset U$.

A subset $C$ of $\F{R}^n$ is \textbf{closed}\index{Close set} if $\F{R}^n - C$ is open.
...
satisfying (2) the \textbf{exterior}\index{Exterior of a set} of $A$, and those satisfying (3) the
...
A collection \C{O} of open sets is an \textbf{open cover}\index{Open cover} of $A$ 
...
special designation: a set A is called \textbf{compact}\index{Compact} if every open
