...
The integral of a 1-form over a 1-chain is often called a \textbf{line integral}. 
...
\begin{problems}
    \problem{ (\textit{Independence of parameterization}).
    ...
    \problem{
        ...
        the \textbf{winding number} of $c$ around 0.
    }
    \problem{
        Recall that the set of complex numbers $\F{C}$ is simply $\F{R}^2$ 
        with $(a,b) = a + b\R{i}$. If $a_1,\cdots,a_n \in \F{C}$ let 
        $f:\F{C}\to \F{C}$ be $f(z)= z^n + a_1z^{n-1} + \cdots+a_n$. Define singular 
        ...
        \begin{enumerate}[label=(\alph*)]
            ...
            \item Using Problem 4-26, prove the \textit{Fundamental Theorem of Algebra}\index{Algebra Fundamental Theorem}: 
    ...
    \problem{
        ...
        for some $\lambda\in\F{R}$ and $g:\F{R}^2-\{0\}\to\F{R}$. \textit{Hint:} If 
        \begin{align*}
    ...
    \problem{
        \begin{enumerate}[label=(\alph*)]
            \item Let $c_1, c_2$ be singular 1-cubes in $\F{R}^2$ with $c_1(0) = c_2(0)$ 
                ...
                $\partial c = c_1- c_2 + c_3 - c_4$, where $c_3$ and $c_4$ are degenerate, 
    ...
    \problem{(\textit{A first course in complex variables.}) 
        If $f: \F{C}\to \F{C}$, define $f$ to be \textbf{differentiable} at $z_0\in \F{C}$ if 
        the limit
        \begin{align*}
        ...
        continuous on $A$, then $f$ is called \textbf{analytic}\index{Analytic function} on $A$.
        \begin{enumerate}[label=(\alph*)]
            ...
            \item If $f$ is analytic on $\{z: |z| < 1\}$, use the fact that $g(z)=f(z)/z$ is 
                ...
                and conclude:\par 
                \qquad\textit{Cauchy Integral Formula:} If $f$ is analytic on $\{z:|z|<1\}$ and 
    ...
    \problem{
        If $F:[0, 1]^2 \to\F{R}^3$ and $s\in [0,1]$ define $F_s:[0,1]\to\F{R}^3$ by 
        $F_s(t)=F(s,t)$. If each $F_s$ is a closed curve, $F$ is called a \textbf{homotopy} 
        ...
        \begin{enumerate}[label=(\alph*)]
            \item If $f,g:[0,1]\to\F{R}$ are nonintersecting closed curves define 
