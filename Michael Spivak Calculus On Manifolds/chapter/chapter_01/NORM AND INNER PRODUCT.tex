...
called the \textbf{Norm}\index{Absolute value} $|x|$ of $x$ and defined by $|x| = \sqrt{(x^1)^2 +\cdots + (x^n)^2}$.
...
\begin{problems}
    ...
    \problem{The quantity $|y-x|$ is called the distance between $x$ and $y$.Prove and 
        interpret geometrically the "triangle inequality": $|z-x| \le |z-y| + |y-x|$.}
    \problem{Let $f$ and $g$ be integrable on $[a, b]$.
        \begin{enumerate}[label={\upshape(\alph*)}]
            \item Prove that 
                \begin{align*}
                    \left|\int_{a }^{b }{f\cdot g }\right| \le \left(\int_{a }^{b }{f^2}\right)^{\frac{1 }{2}} \cdot \left(\int_{a }^{b }{g^2}\right)^{\frac{1 }{2}}
                \end{align*} 
        ...
        \end{enumerate}}
    \problem{A linear transformation $T:\F{R}^n \to \F{R}^m$ is \textbf{Norm preserving} if $|T(x)| = |x|$, and 
        \textbf{inner product preserving} if $\langle Tx, Ty\rangle = \langle x, y\rangle$.
        \begin{enumerate}[label={\upshape(\alph*)}]
    ...
    \problem[*]{Let $\left(\F{R}^n\right)^*$ denote the dual space of the vector $\F{R}^n$. If $x\in \F{R}^n$,
    ...
    \problem[*]{If $x, y\in \F{R}^n$, then $x$ and $y$ are called \textbf{perpendicular} (or \textbf{orthogonal}) 
